                        ...
                        mongoDatabase.collection('MeetUp_Request').find({ $or: [{ "roomId":roomId },{ "roomId":roomIdAlt }]}).toArray(function(err, result1){
                            if(result1.length==0)
                            {
                                ...
                                mongoDatabase.collection('MeetUp_Request').insertOne(myobj,function(response){
                                    var message = "Friend request sent by "+currObj.body.fromUserName

                                    var notifyObj = {"notificationId":id,"fromUserId": currObj.body.fromUserId, "toUserId": currObj.body.toUserId,"requestStatus": currObj.body.requestStatus,
                                    "fromUserName": currObj.body.fromUserName,"toUserName": currObj.body.toUserName,"message":message,"meetupRequestId":id}
                                    mongoDatabase.collection('Notification_Table').insertOne(notifyObj,function(notificationResult){
