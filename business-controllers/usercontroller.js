                ...
                objMongoConnector.execQuery(function(conn,mongoDatabase){
                    ...
                    if(currObj.body.regdType == 'R')
                    {
                        mongoDatabase.collection('User_Registration').find().sort({ userId: -1 }).limit(1).toArray(function(err, result) {
                            ...
                            "userSubCourse":currObj.body.userSubCourse,"profilePic":""}
                    ...
                    else if(currObj.body.regdType == "U")
                    {
                        ...
                        var updObj;
                        if(currObj.body.profilePic!='')
                        {
                            var path ='E:\\CSM_AI_Projects\\foodbuddy\\public\\userPic\\';
                            var timestamp = Number(new Date());
                            var optionalObj = {'fileName': timestamp, 'type':'png'};
                            base64ToImage(currObj.body.profilePic,path,optionalObj);
                            var imageName = timestamp+".png"
                            updObj = {"name": currObj.body.name,"foodPreference": currObj.body.foodPreference,
                            "interest": currObj.body.interest,"bio":currObj.body.bio,"profilePic":imageName}
                        }
                        else
                        {
                            updObj = {"name": currObj.body.name,"foodPreference": currObj.body.foodPreference,
                            ...
                            objMongoConnector.findAndUpdate(updParam, updObj, "User_Registration",function(updResponse){
                                ...
                                var count = 0;
                                if(currObj.body.freeTime.length > 0)
                                {
                                    objMongoConnector.delete(updatedUserId, "User_Free_Time","m",function(response1){
                                        for(let i=0; i< currObj.body.freeTime.length; i++)
                                        {
                                            ...
                                            objMongoConnector.insert(myTimeSlotObj,"o", "User_Free_Time",function(response2){
                                                totalOutput.push(response2.output)
                                                count++;
                                                if(count == currObj.body.freeTime.length)
                                                {
                                                    let res = []
                                                    var obj= updResponse.data.value
                                                        obj.name = updObj.name
                                                        obj.foodPreference = updObj.foodPreference
                                                        obj.interest = updObj.interest
                                                        obj.bio = updObj.bio
                                                        obj.profilePic = updObj.profilePic
                                                        obj.freeTime = totalOutput;
                                                        res.push(obj)
